...
class FusionSC_Blog {
	...
	function render( $args, $content = '') {
		...
		if ( self::$args['layout'] == 'grid' &&
			...
			$html .= sprintf( '<style type="text/css">.fusion-blog-shortcode-%s .fusion-blog-layout-grid .fusion-post-grid{padding:%spx;}</style>', $this->blog_sc_counter, $defaults['blog_grid_column_spacing'] / 2 );
		}
	...
	function posts_container_attr() {
		...
		if ( self::$args['layout'] == 'grid' ) {
			 ...
			 $attr['style'] = sprintf( 'margin: %spx 0 %spx;', $negative_margin, $negative_margin );
		}
	...
	function loop_header() {
		...
		$pre_title_content .=  sprintf( '<div %s>', FusionCore_Plugin::attributes( 'fusion-post-content' ) );
