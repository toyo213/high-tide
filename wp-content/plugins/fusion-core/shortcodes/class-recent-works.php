...
class FusionSC_RecentWorks {
		...
		while ( $recent_works->have_posts() ) { 
			...
			if ( $layout == 'carousel' ) {
				...
				if ( $carousel_layout == 'title_on_rollover' ) {
					$image = avada_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE, FALSE, FALSE, 'disable' );
				// Title below image layout
				} else {
					$image = avada_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE, FALSE, FALSE, 'disable', 'disable' );
				...
				// Get the post image
				$image = avada_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE );
	...
	function attr() {
		...
		if( self::$args['layout'] == 'grid' ) {
			$attr['class'] .= ' fusion-portfolio-grid';
		}
